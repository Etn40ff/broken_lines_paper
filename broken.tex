...
The first basis we consider is the greedy basis of \cite{LLZ}.  Every cluster algebra is contained in the ring of Laurent polynomials in the cluster variables of any of its clusters.  The recently-confirmed positivity conjecture, proved in the rank 2 case \cite{LS,Rup2} and in the general case in \cite{LS2,GHKK}, asserts that the coefficients of the Laurent expansion of any cluster variable are positive integers.  \sayGM{I find the phrasing "builds this property into any canonical basis" awkward, but am happy to leave as is if everyone else is happy with this paragraph. (Gregg)} The definition of the greedy basis builds this property into any canonical basis element by attempting to describe the minimal conditions for a Laurent polynomial in an initial cluster to be both positive and Laurent in the two adjacent clusters.  The resulting coefficients turn out to enumerate combinatorial objects called compatible pairs related to maximal Dyck paths.
...
\begin{theorem} 
  ...
  $M_{\RR}$ with starting point in the first quadrant \sayGM{Just checking: is this supposed to be 'second quadrant?' (Gregg)} and endpoint in one of the
...
\begin{defn} 
  ...
  \end{itemize}
\end{defn}
We refer to $m(\ell)\in \mathbb{Z}^2$ as the \emph{momentum} of that domain
of linearity.
...
The proof will be to show that $\Theta_{q,m}^{\mathbf{d}}$ is a pointed element with the same Newton polygon as $x[-m_1,-m_2]$.  By Scholium \ref{sch}, this is already enough to show that $\Theta_{q,m}^\mathbf{d}=x[-m_1,-m_2]$.
