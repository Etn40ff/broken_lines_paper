...
\begin{theorem}
\cite{LS,Rup} \sayDR{Lee-Schiffler only establishes positivity for $b=c$.} Each cluster variable of $\cA(b,c)$ is positive.
\end{theorem}
...
\begin{defn}
  ...
  \begin{itemize}
    ...
    \item 
      ...
      for some $c_k \in \ZZ$. 
  \end{itemize}
  \sayDR{Does this clash with the use of ``support'' from section 2?} The set $\fd \subset M_{\mathbb{R}}$ is called the \emph{support} of the wall
  $(\fd, f_{\fd})$.
\end{defn}
...
Given a wall $(\fd, f_{\fd})$ and a direction $v\in M$ transversal to $\fd$, we associate the element $\theta_{v,\fd}\in
...
\]
\sayDR{The $k$ here looks like an exponent, does anyone oppose moving it to a subscript?} Then we define
\[
  \theta_{\gamma, \fD} := \lim_{k \rightarrow \infty} \theta ^k_{\gamma, \fD}. 
\]\sayDR{If well-definedness is a potential issue we should mention that here.}
...
which appear for $bc\ge4$ can be found in \cite[Example 1.30]{GHKK}, where the description
is based on the description of a related scattering diagram given in
\cite{GP} \sayDR{Is the second clause in this sentence necessary?}. We summarize the crucial points here.
...
\sayDR{The next two paragraphs seem to be quoting a non-trivial result, we should probably give a reference.} Now consider the two linear involutions $S_1$ and $S_2$ given by
\[
  S_1 =  
  \begin{pmatrix}
    -1 & -b \\
...
\]
is a wall in $\fD_{(b,c)}$. These considerations gives us a recipe to produce
elements of $\fD_{(b,c)} \smallsetminus \fD_{\mathrm{in},(b,c)}$: it is enough to apply
alternatively $S_1$ and $S_2$ to the walls (\ref{eq:first-walls}). \sayDR{This sentence makes it sound like all walls are obtained in this way.}

We need to distinguish two cases \sayDR{The exposition would probably be slightly cleaner if we distinguish three cases instead.}. If $bc<4$ this procedure will construct, in
...
is very well-behaved and familiar in the theory of cluster algebras \sayDR{Should we also mention root systems here?}. This
...
and the corresponding cluster algebras. \sayDR{The statement of the theorem does not a priori have anything to do with cluster algebras, probably something more should be said here if we want to keep this sentence.}
\begin{theorem} 
  \label{univLaurent} 
  Let $\fD:=\fD_{(b,c)}$ be as constructed above and consider a Laurent polynomial $f \in \ZZ[M]$. For any path $\gamma$ for which $\theta_{\gamma,\fD}$ is
  well-defined, $\theta_{\gamma,\fD}(f)$ can be viewed as an element of
  $\ZZ[[x_1,x_2^{-1}]]$ localized at $x_1x_2^{-1}$ \sayDR{Is this sentence a part of the theorem or should it be moved above?}.  If for any path $\gamma$ in
  $M_{\RR}$, with starting point in the first quadrant and endpoint in one of the
  chambers of $\fD$, $\theta_{\gamma,\fD}(f)$ lies in $\ZZ[M]$, then $f$ is a universal
  Laurent polynomial.
\end{theorem}
...
\begin{theorem} $ $
  \begin{enumerate}
    \item
      ...
      path joining $q$ to $q'$, then $\theta_{\gamma, \fD }(\Theta_{q,m}) = \Theta_{q', m}$. 
    \item Take $\fD=\fD_{(b,c)}$. 
      \begin{enumerate}
        \item If $q$ lies in the interior of a chamber of $\fD$, then $\Theta_{q,m}$ is a Laurent polynomial for any $m$.
        \item If $q$ lies in the interior of the first quadrant, then $\Theta_{q,m}$ is a universal Laurent polynomial for any $m$.
        \item If $q$ and $m$ lie in the interior of the same chamber of $\fD$, then $\Theta_{q,m}=x^{m}$.
      \end{enumerate}
  \end{enumerate}
\end{theorem}

\begin{proof}
  (1) is a main result of \cite{CPS}, quoted in this context in \cite[Theorem~3.5]{GHKK}.  (2a) is \cite[Example~7.18]{GHKK} \sayDR{It isn't clear to me at all how \cite[Example~7.18]{GHKK} is related to the claim.} with $\Theta$ taken to be the subset of $M$ consisting of those $m$ for which $\Theta_{q,m}$ is a Laurent polynomial for some $q$ in the interior of a chamber of $\fD$ (and hence by \cite[Proposition~7.1]{GHKK}, all chambers).  Next, (2b) follows from (2a) by Theorem \ref{univLaurent} and (1).  Finally, (2c) follows from \cite[Proposition~3.8]{GHKK} using the argument of \cite[Theorem~4.8]{GHKK} \sayDR{The statement of \cite[Theorem~4.8]{GHKK} is a much heftier result than what we are proving, maybe it would be good to extract the relevant portion of the proof and present it here.} in the case that the chamber is not the positive quadrant.
\end{proof}

\begin{remark} 
  \label{rk:theta functions g-vector}
  If $m\in M$ lies in one of the chambers of $\fD_{(b,c)}$ and $q$ lies in
  the first quadrant, then from (1) and (2c) above we see that
...
\begin{example}
  ...
  $\Theta _{q, (2,-2)}$ is a cluster monomial.
