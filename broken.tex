...
\saySS{This section was written by Dylan independently while Mark, Mandy and
  ...
  conventions are consistent. I am ready to bet that we need to take a transpose
  somewhere.}
...
proven by Fomin and Zelevinsky \cite{FZ}.
\begin{theorem}\cite[Theorem~3.1]{FZ} 
...
\begin{theorem}\label{th:pointed supports}\cite[Proposition~4.1 and Theorem~???]{LLZ,LLZ2}
  \saySS{We miss a reference here.}
...
\begin{enumerate}
  ...
  \item If $0<ba_2\leq a_1$, then $R_{a_1,a_2}=\big\{(p,q)\in\ZZ_{\ge0}^2\big|\; p\le a_{2},\; q\le a_{1}-bp\big\}$.
  \item If $0<ca_1\leq a_2$, then $R_{a_1,a_2}=\big\{(p,q)\in\ZZ_{\ge0}^2\big|\; q\le a_{1},\; p\le a_{2}-cq\big\}$.
  ...
  &\hspace{1.5em}\bigcup\Big\{(0,a_1),(a_2,0)\Big\}.
  \endaligned
  $$
  \sayDR{I do not like how these allign; we should fix this in the final
    version.}
\end{enumerate}
Moreover, if $x\in\cA(b,c)$ is any element pointed at $(a_1,a_2)$ whose pointed
support is equal to $R_{a_1,a_2}$, then $x=x[a_1,a_2]$. 
\end{theorem}
...
\begin{remark}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=.7]
      ...
      \begin{scope}[shift={(.5, 5.5)}]
        ...
        \draw[->] (0,0) -- (2.5,0) node[above] {\tiny $p$};
        \draw[->] (0,0) -- (0,2.5) node[left] {\tiny $q$};
        \fill (0,0) circle (2pt);
  ...
  \begin{center}
    \begin{tikzpicture}[scale=.7]
      ...
      \begin{scope}[shift={(11,0)}]
        ...
        \draw[color=white,fill=white] (1.5,1.8) circle (2pt);
        \fill [black!10]  (0,3)--(1.5,1.77)--(2.5,0)--(0,0)--(0,3);
        \draw (1.5,1.8) circle (2pt);
        \draw (0,0) node[anchor=east] {\tiny$O$};
      ...
      \begin{scope}[shift={(16,0)}]
        ...
        \draw (2.2,0) node[anchor=south west] {\tiny$A$};
        \draw (.7,.7) circle (2pt);
        \draw (.7,.7) node[anchor=south west] {\tiny$B$};
        \draw (0,3) node[anchor=east] {\tiny$C$};
        \draw[->] (0,0) -- (3.5,0) node[above] {\tiny $p$};
        \draw[->] (0,0) -- (0,3.5) node[left] {\tiny $q$};
        \fill (0,0) circle (1.5pt);
  ...
  \end{center}
\end{remark}
\sayDR{Please remember to do the small cosmetic adjustments that will make this
  figure much awsomer.}
...
Let $N = \ZZ^2$ be a lattice and $M = \Hom (N, \ZZ)$. We will write elements of $N$ and of $M$ both as tuples, this should not lead to any confusion. Write $M_{\RR} = M\otimes\RR$, $N_{\RR} = N\otimes\RR$. Take $\Bbbk$ to be a field of characteristic 0.  For $m\in M$ we will write $A^m$ for the standard basis vector in $\Bbbk[M]$.  For notational convenience we will write $A_1=A^{(1,0)}$ and $A_2=A^{(0,1)}$, thus we may identify $A^m$ with $A_1^{m_1}A_2^{m_2}$.
...
\begin{defn}
  ...
  \begin{itemize}
    ...
    \item 
      $\fd \subset M_{\mathbb{R}}$ is either a ray $\RR_{\le 0} m_0$ or a line
      $\RR m_0$ with $m_0\in \sigma \cap(M\setminus 0)$;

    \item 
      $f_{\fd} \in \widehat{\Bbbk [P]}$ is such that 
      \[ 
        f_{\fd} = f_{\fd}(A^{m_0}) = 1 + \sum_{k\geq 1} c_k A^{k m_0},
      \] 
...
\[
  \theta_{v,\fd} (A^m) := A^m f_{\fd}^{\langle m, n \rangle }, 
\]
...
that  $f_\fd(A^{S(m_0)})$ is not an element of $\widehat{\Bbbk [P]}$).
...
\begin{defn} 
  ...
  number of domains of linearity, along with a choice of monomials $c_L A^{m_L}
  \in \Bbbk[M]$ one for each domain of linearity $L \subseteq ( - \infty, 0]$ of
  $\gamma$. 
  
  The path $\gamma$ and the monomials $c_L A^{m_L}$ need to satisfy the
  ...
  \begin{itemize}
    ...
    \item 
      ...
      $c_{L'}A^{m_{L'}}$ is a term in $\theta_{-m_L, \fd} (c_L A^{m_L})$.  
  \end{itemize}
\end{defn}

We are finally ready to introduce the main player of our discussion.
\begin{defn}
  Let $\fD, m, Q$ as in Definition~\ref{brokendef}. For a broken line $\gamma$
  with initial slope $m$ and endpoint $Q$, denote by 
  \[
    \Mono (\gamma) = c(\gamma)A^{F(\gamma)}
  \] 
  the monomial $c_L A^{m_L}$ attached to the last domain of linearity $L$ of
...
\begin{example}
  ...
  \]
  In this scattering diagram $\fD_{(2,2)}$, the ray with slope $f_1-f_2$ does
...
First of all $\big( \RR f_2, 1+A_2^c\big)$ is the only wall of $\fD_{(b,c)}$
...
biggest slope in $\fD_{(b,c)}\setminus\fD_{\mathrm{in},(b,c)}$; its image is
$\big( \RR_{\le0} f_2, 1+A_2^c\big)$.

\begin{defn}
  $\fD_{\gr,(b,c)}$ is the scattering diagram obtained from
  $T(\fD_{(b,c)})$ by replacing 
  \begin{itemize}
    \item
...
\begin{theorem}
  ...
  $T$ defines a one-to-one correspondence between broken lines in $\fD_{(b,c)}$
...
\begin{proof}
  ...
  along $\RR (-f_1)$. So $c_{L_2} A^{m_{L_2}}$ is a term in 
  \[
    \theta_{-m_{L_1},\RR(-f_1)} (c_{L_1} A^{m_{L_1}})
    = 
    c_{L_1} A^{m_{L_1}} (1+A_1^{-b}) ^{|\langle  m_{L_1}, e_2  \rangle|}.
  \]
  ...
  $c_{L_2}A^{T_+(m_{L_2})} =c_{L_2}A^{m_{L_2}}$ attached to $L_2$ in
  $T\circ\gamma$ to satisfy the bending rule, it must be a term in
  \[
    \theta_{-T_-(m_{L_1}),\RR f_1} (c_{L_1} A^{T_-(m_{L_1})}). 
  \]
  Since 
  \[
  ...
  \begin{align*} 
    \theta_{-T_-(m_{L_1}),\RR f_1} (c_{L_1} A^{T_-(m_{L_1})}) 
    & =
    c_{L_1} A^{T_-(m_{L_1})} (1+A_1^b) ^{-\langle T_-(m_{L_1}), e_2\rangle}
    \\
    & = 
    c_{L_1} A^{m_{L_1}} A_1^{b\langle m_{L_1}, e_2\rangle} 
    (1+A_1^b)^{- \langle m_{L_1}, e_2 \rangle} 
    \\
    & = 
    c_{L_1} A^{m_{L_1}} (1+A_1^{-b}) ^{-\langle  m_{L_1}, e_1  \rangle}.
  \end{align*}
...
\begin{prop}
  ...
  \[
    \vartheta^{\gr}_{Q, m}=A^{m}(1+f(A_1,A_2))
  \]
