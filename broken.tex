...
\begin{theorem}\label{th:pointed supports}\cite[Proposition~4.1 and Theorem~???]{LLZ,LLZ2}
  ...
  \sayDR{Rewrite this in terms of the Newton polytope}
...
\begin{enumerate}
  ...
  \sayDR{I do not like how these allign; we should fix this in the final
    version.}
\end{enumerate}
\saySS{We miss a reference here. It should be \cite[Corollary 3.5]{LLZ2}}
...
\begin{example} 
  ...
  the two walls
  \[ 
    \left( \RR_{\leq 0} (-1,1), 1+A_1^{-2}A_2^2 \right)
    \quad \mbox{and} \quad
    \left( \RR_{\leq 0} (-2,1), 1+A_1^{-2}A_2 \right).
  \]
\end{example}
...
\begin{equation}
  ...
  S_2(\RR_{\leq 0}(-1,0),1+A_1^{-b})
  %
  \quad
  \mbox{and}
  \quad
  %
  S_1(\RR_{\leq 0}(0,1),1+A_2^c)
\end{equation}
are walls in $\fD_{(b,c)} \backslash \fD_{\mathrm{in},(b,c)}$ (even though
neither 
\[
  (\RR_{\leq 0}(-1,0),1+A_1^{-b})
  %
  \quad
  \mbox{nor}
  \quad
  %
  (\RR_{\leq 0}(0,1),1+A_2^c)
\]
...
\[ 
  \mathcal{H}_{+} := 
  \{ m \in M_{\mathbb{R}}\, |\, m_2 \geq 0 \} 
  \quad
  \mbox{and}
  \quad
  \mathcal{H}_{-} := 
  \{ m \in M_{\mathbb{R}} \,|\, m_2 \leq 0 \}.
\]
...
Indeed $\big(\RR_{\leq0}(-b,1),1+A_1^{-bc}A_2^c\big)$ is the wall with the
...
\begin{proof}
  ...
  \[
    \theta_{-m(L),\RR(-1,0)} (c(L) A^{m(L)})
    = 
    c(L) A^{m(L)} (1+A_1^{-b}) ^{|m_2(L)|}.
  \]
  ...
  \[
    \theta_{-T_-(m(L)),\RR (1,0)} (c(L) A^{T_-(m(L))}). 
  \]
  Since the second component of $T_-(m(L))$ is $m_2(L)$ we get
  \begin{align*} 
    \theta_{-T_-(m(L)),\RR (1,0)} (c(L) A^{T_-(m(L))}) 
    & =
    c(L) A^{T_-(m(L))} (1+A_1^b) ^{-m_2(L)}
    \\
    & = 
