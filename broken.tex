...
\begin{defn} 
  ...
  The path $\gamma$ and the monomials $c(L) x^{m(L)}$ need to satisfy the
  ...
  \begin{itemize}
    ...
    \item 
      ...
      $c(L')x^{m(L')}$ is a term in 
      \[
        \theta_{-m(L), \fd} \left(c(L) x^{m(L)}\right).
      \]
  \end{itemize}
\end{defn}
...
\begin{proof}
  ...
  along $\RR (-1,0)$. So $c(L') x^{m(L')}$ is a term in 
  \[
    \theta_{-m(L),\RR(-1,0)} \left(c(L) x^{m(L)}\right)
    = 
    c(L) x^{m(L)} \left(1+x_1^{-b}\right) ^{|m_2(L)|}.
  \]

  First, assume $\gamma$ passes from $\mathcal{H}_-$ to $\mathcal{H}_+$. In this
  case, we have $m_2(L) < 0$. Now in order for the monomial
  $c(L')x^{T_+(m(L'))} =c(L')x^{m(L')}$ attached to $L'$ in
  $T\circ\gamma$ to satisfy the bending rule, it must be a term in
  \[
    \theta_{-T_-(m(L)),\RR (1,0)} \left(c(L) x^{T_-(m(L))}\right). 
  \]
  Since the second component of $T_-(m(L))$ is $m_2(L)$ we get
  \begin{align*} 
    \theta_{-T_-(m(L)),\RR (1,0)} \left(c(L) x^{T_-(m(L))}\right) 
    & =
    c(L) x^{T_-(m(L))} \left(1+x_1^b\right) ^{-m_2(L)}
    \\
    & = 
    c(L) x^{m(L)} x_1^{b m_2(L)} 
    \left(1+x_1^b\right)^{-m_2(L)} 
    \\
    & = 
    c(L) x^{m(L)} \left(1+x_1^{-b}\right) ^{-m_1(L)}.
  \end{align*}
