...
\begin{theorem}
\cite[for $b=c$]{LS}, \cite{Rup}\sayDR{Lee-Schiffler only establishes positivity for $b=c$.}\sayGM{Does this look better to you? (Gregg)} Each cluster variable of $\cA(b,c)$ is positive.
\end{theorem}
...
\sayGM{Add definitions of d-vectors and g-vectors in this section. (Gregg, Dylan)}
...
\begin{defn}
  ...
  \begin{itemize}
    ...
    \item 
      ...
      for some $c_k \in \ZZ$. 
  \end{itemize}
  \sayDR{Does this clash with the use of ``support'' from section 2?}\sayGM{I think this is unavoidable.  Added parenthetical comment. (Gregg)} The set $\fd \subset M_{\mathbb{R}}$ is called the \emph{support} of the wall
...
\]
transverse to each wall of $\fD$ that it crosses and with endpoints not in the support of $\fD$. We define the \textit{path-ordered product} $\fp_{\gamma, \fD}$ along $\gamma$ as follows. For each power $k \geq 1$, let  
\[
...
\]
\sayDR{The $k$ here looks like an exponent, does anyone oppose moving it to a subscript?}\sayGM{I think the subscript is already too crowded.  How do like parenthetical exponents instead? (Gregg)} Then we define
\[
...
\sayDR{The next two paragraphs seem to be quoting a non-trivial result, we should probably give a reference. [Mandy: ref added]} Now consider the two linear involutions $S_1$ and $S_2$ given by
\[
  S_1 =  
  \begin{pmatrix}
    -1 & -b \\
...
alternatively $S_1$ and $S_2$ to the walls (\ref{eq:first-walls}) to produce some walls described below. 
\sayDR{This sentence makes it sound like all walls are obtained in this way. [Mandy: rewrote]}
...
is very well-behaved and familiar in the theory of cluster algebras \sayDR{Should we also mention root systems here?}. \
\sayMC{if someone are interested in typing, they can :P} This
...
\sayDR{The statement of the theorem does not a priori have anything to do with cluster algebras, probably something more should be said here if we want to keep this sentence.}\sayGM{I attempted a rewrite, what do you think? (Gregg)}
\begin{theorem} 
  \label{univLaurent} 
  Let $\fD:=\fD_{(b,c)}$ be as constructed above and consider a Laurent polynomial $f \in \ZZ[M]$. For any path $\gamma$ for which $\fp_{\gamma,\fD}$ is
  well-defined, $\fp_{\gamma,\fD}(f)$ can be viewed as an element of
  ...
  Laurent polynomial.
\end{theorem}
\sayMC{This theorem is saying $f$ is a universal Laurent poly for cluster. So it is related. The sentence you are quoting can be pulled out in the theorem but it may be good to put them together.}
...
\begin{proof}
  (1) is a main result of \cite{CPS}, quoted in this context in \cite[Theorem~3.5]{GHKK}.  (2a) is \cite[Example~7.18]{GHKK} \sayDR{It isn't clear to me at all how \cite[Example~7.18]{GHKK} is related to the claim.} with $\vartheta$ taken to be the subset of $M$ consisting of those $m$ for which $\vartheta_{q,m}$ is a Laurent polynomial for some $q$ in the interior of a chamber of $\fD$ (and hence by \cite[Proposition~7.1]{GHKK}, all chambers).  Next, (2b) follows from (2a) by Theorem \ref{univLaurent} and (1).  Finally, (2c) follows from \cite[Proposition~3.8]{GHKK} using the argument of \cite[Theorem~4.8]{GHKK} \sayDR{The statement of \cite[Theorem~4.8]{GHKK} is a much heftier result than what we are proving, maybe it would be good to extract the relevant portion of the proof and present it here.} in the case that the chamber is not the positive quadrant.
\end{proof}
\sayMC{going to rewrite it}
\begin{remark} 
...
\sayDR{We never actually give a definition of either $g$-vectors or $d$-vectors, maybe this should be added to section 2.}
\sayGM{Agreed. (Gregg)}
...
walls of $\fD_{(b,c)}\smallsetminus\fD_{\mathrm{in},(b,c)}$ to the third quadrant \sayDR{It seems like a little more details are needed here, we also need to refer to this in the proof of Prop. 4.4 so it might be good to state as a lemma.}. \sayMC{We can do it. The reason is only that all the rays are between $(b,-1)$ and $(0,-1)$ and as the two 'boundaries' go to the third quadrant, all the rays inside the 'cone' will follow.}
...
The proof will be to show that $\vartheta_{q,m}^{\mathbf{d}}$ is a pointed element with the same Newton polygon \sayGM{1) do we mean 'pointed element', same issue as in proof of Thm \ref{th:Newton polygons}  2) need to use 'support' rather than 'Newton polygon'.}\sayGM{ the term "Newton polygon" also still needs to be updated here. (Gregg)} as $x[-m_1,-m_2]$.  By Scholium \ref{sch}, this is already enough to show that $\vartheta_{q,m}^\mathbf{d}=x[-m_1,-m_2]$.
...
\begin{proof}
The lemma is straightforward except for broken lines with initial exponent $(m_1,m_2)$ with $m_1,m_2>0$.\sayGM{Should this be $m_1, m_2 < 0$? (Gregg, Mark)}
...
We can now constrain the possible final exponent of a broken line, which will be used to bound the Newton polygon \sayGM{"Newton polygon" should be changed to "support" or "lattice quadrilateral containing the support" (Gregg)} of the corresponding theta function.
...
If $m=(m_1,m_2)$ such that $m_1\leq0$ or $m_2\leq0$\sayGM{Should this be $m_1 \geq 0$ or $m_2 \geq 0$? (Gregg, Mark)}, then $\vartheta_{q,m}^\mathbb{d}$ is the cluster monomial $x[-m_1,-m_2]$, by Remark \ref{rk:theta functions g-vector}.
