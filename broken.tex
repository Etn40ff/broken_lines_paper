...
\address[Dylan Rupel]{University of Notre Dame (formerly Northeastern University)}
\email{drupel@nd.edu}
...
\begin{theorem}
\cite{LS,Rup}\sayGM{Does this look better to you? (Gregg)}\sayDR{I only wanted to let people know why I added the extra reference, I don't think it needs to be in the paper.} Each cluster variable of $\cA(b,c)$ is positive.
\end{theorem}
...
Let $\fD$ be a scattering diagram and $\gamma: [0,1] \rightarrow M_{\mathbb{R}} \smallsetminus  \{0 \}$ a smooth immersion with endpoints not in the support of $\fD$ which is transverse to each wall of $\fD$ that it crosses. We define the \textit{path-ordered product} $\fp_{\gamma, \fD}$ along $\gamma$ as follows. For each power $k \geq 1$, let  
\[
