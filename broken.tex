...
%the \emph{pointed support} of $x[a_1,a_2]$.  \Cref{th:greedy} is proven using the following characterization of the greedy elements by their
%pointed supports.
...
The proof will be to show that $\Theta_{q,m}^{\mathbf{d}}$ is a pointed element with the same Newton polygon as $x[-m_1,-m_2]$.  By Theorem \ref{th:pointed supports}, this is already enough to show that $\Theta_{q,m}^\mathbf{d}=x[-m_1,-m_2]$.
...
\sayGM{This proof needs: a restatement of Theorem \ref{th:pointed supports} which doesn't say `pointed' and only requires the right inequalities.}
...
Since $\Theta_{q,m}^\mathbf{d}\in \mathcal{A}_{b,c}$, Theorem \ref{th:pointed supports} implies that $\Theta_{q,m}^\mathbf{d}$ is a scalar multiple of $x[-m_1,-m_2]$. 
