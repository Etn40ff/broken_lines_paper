...
On the other hand, the chamber structure (i.e. the collection of cones in which the rays cut the plane)
\saySS{Mark suggests explaining what we mean here by chamber structure; do you
  like this attempt? }
one sees outside of the irrational cone
...
%\sayDR{Is it true that the resulting scattering functions (the ones we don't toss out anyway) will be elements of $\widehat{\ZZ[P_{\sigma^\bfd}]}$?  The exponent of $x_1$ on the original functions is always $\le0$ but $T$ can only make this exponent more negative.  I think this is the sign convention issue we had before rearing its ugly head again.}
...
\saySS{Mark pointed out that the implication holds only if both $m_1$ and
  $m_1+kw_1$ have the same sign. Under the assumption we are dealing with, this
  is the same as requiring that $m_1 < -kw_1$ with both $k$ and $w_1 >0$. Intuitively this means that after
  a bend we need to continue to move to the right but I do not see how to
