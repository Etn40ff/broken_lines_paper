...
Recall from the previous section the lattice $M \cong \ZZ^2$ such that $\ZZ[M] \cong \ZZ[x_1^{\pm 1}, x_2^{\pm 1}]$.  We write $N = \Hom (M, \ZZ)$ for its dual lattice, $M_{\RR} := M\otimes\RR$, $N_{\RR} := N\otimes\RR$, and we denote the standard pairing of $m\in M$ and $n\in N$ by $m \cdot n$.  Given a strictly convex rational cone $\sigma \subsetneq M_{\RR}$, we write $P_{\sigma}=\sigma \cap M$ \sayDR{Since $\sigma$ occasionally changes should we write something like $P_\sigma$ instead?}. \sayMC{get it changed!} \sayDR{If the change was warranted it should probably be made everywhere...\emph{if}} \sayMC{I am sorry that I missed some. How about you help me to check where I missed and change it?} Let $\widehat{\ZZ[P_{\sigma}]}$ denote the completion of the monoid ring $\ZZ[P_{\sigma}]$ at the maximal monomial ideal $\fm$ generated by $\left\{x^m \,|\, m\in P_{\sigma}\smallsetminus\{0\}\right\}$.
...
\begin{defn}
  ...
  \begin{itemize}
    ...
    \item 
      $f_{\fd} \in \widehat{\ZZ [P_{\sigma}]}$ is such that 
      \[ 
...
Let $\fD$ be a scattering diagram and $\gamma: [0,1] \rightarrow M_{\mathbb{R}} \smallsetminus  \{0 \}$ a smooth immersion with endpoints not in the support of $\fD$ which is transverse to each wall of $\fD$ that it crosses \sayDR{It seems to make sense to introduce some terminology to refer to such paths since we need it so often, maybe call them \emph{regular with respect to $\fD$}?}\sayMC{Yes! but it seems this is almost the only place we use the curve} \sayDR{We actually refer to these curves every time we say something like ``...if it is defined...'' adding this language would avoid such opaque statements}\sayMC{How about you make the change?}. We define the \textit{path-ordered product} $\fp_{\gamma, \fD}$ along $\gamma$ as follows. For each power $k \geq 1$, let  
\[
