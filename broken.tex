...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\title{A Rank Two Haiku:\\ The Greedy Basis Equals \\ The Theta Basis}
\author{CCGMMRSW}
...
\begin{theorem}\label{th:pointed supports}(\cite[Proposition~4.1]{LLZ} and \cite[Corollary 3.5]{LLZ2})
  ...
  \begin{enumerate}
    ...
    &\bigcup\bigg\{(p,q)\in\ZZ_{\ge0}^2\bigg|\; 0\le q<\frac{a_2}{c},\; p+\Big(c-\frac{ca_1}{ba_2}\Big)q<a_2\bigg\}\\
    &\hspace{1.35em}\bigcup\Big\{(0,a_1),(a_2,0)\Big\}.
    \endaligned
    $$
  \end{enumerate}
  Moreover, if $x\in\cA(b,c)$ is any element pointed at $(a_1,a_2)$ whose Newton polytope is contained in that of $x[a_1,a_2]$, then $x=x[a_1,a_2]$. 
\end{theorem}
\begin{proof}
  The first part is the content of \cite[Proposition~4.1]{LLZ} and \cite[Corollary 3.5]{LLZ2}.

  Suppose $x\in\cA(b,c)$ is pointed at $(a_1,a_2)$ and that the Newton polytope of $x$ is contained in that of $x[a_1,a_2]$.  If $x\ne x[a_1,a_2]$, then there exists a monomial $x_1^{-a'_1}x_2^{-a'_2}$ \sayDR{exponents chosen for notational convenience} appearing in $x$ with a different coefficient than in $x[a_1,a_2]$.  Since $x$ is pointed at $(a_1,a_2)$, for any such monomial we have $a'_1<a_1$ or $a'_2<a_2$.  Choose a monomial with $(a'_1,a'_2)$ maximal in lexicographic order.  Then in the greedy basis expansion of $x$ the element $x[a'_1,a'_2]$ must appear with nonzero coefficient.

  There are two case to consider.  If $\frac{a'_2}{a'_1}\le\frac{a_2}{a_1}$, then we consider the point $(-a'_1,-a'_2+ca'_2)$
\end{proof}
