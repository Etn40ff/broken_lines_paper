...
The proof will be to show that $\Theta_{m}^{\mathbf{d}}$ is a pointed element with the same Newton polytope as $x[-m_1,-m_2]$.  By Theorem \ref{thm: Dylan}, this is already enough to show that $\Theta_{m}^\mathbf{d}=x[-m_1,-m_2]$.
...
\sayGM{This proof needs: a restatement of Theorem \ref{thm: dylan} which doesn't say `pointed' and only requires the right inequalities.}
...
Since $\Theta_{m}^\mathbf{d}\in \mathcal{A}_{b,c}$, Theorem \ref{thm: dylan} implies that $\Theta_m^\mathbb{d}$ is a scalar multiple of $x[-m_1,-m_2]$. 
