...
generate.  Each pair $\{x_k, x_{k+1}\}$ is called a \emph{cluster} and each monomial $x_k^mx_{k+1}^n$ with $m, n > 0$ is called a \emph{cluster monomial}.  Later we will denote by $M$ a rank 2 lattice with an isomorphism $\ZZ[M] \cong \ZZ[x_1^{\pm1},x_2^{\pm1}]$.  Writing elements of $\ZZ[M]$ as $x^m$ for $m \in M$, this induces an isomorphism $M \cong \ZZ^2$, $m \mapsto (m_1,m_2)$ via $x^{(m_1,m_2)} = x_1^{m_1}x_2^{m_2}$.
...
$\cA(b,c)$.
Following Example 1.30 of \cite{GHKK}, we
