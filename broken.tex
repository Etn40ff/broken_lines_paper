...
\[ \{m\in M \mid c_m\neq 0\} \]
and the \emph{Newton polygon} of $f$ is the convex hull in $M\otimes\mathbb{R}$ of the support of $f$.  

\begin{theorem}\label{th:pointed supports}\cite[Proposition~4.1]{LLZ}, \cite[Corollary 3.5]{LLZ2}
...
\end{figure}
\begin{proof}
  The first part is the content of \cite[Proposition~4.1]{LLZ} and \cite[Corollary 3.5]{LLZ2}.  \sayGM{*** We need this proof to care about the dotted lines rather than the Newton polygon. (gregg)}
...
%We fix lattices $N = \ZZ^2$ and $M = \Hom (N, \ZZ)$. We write their elements as tuples and denote the standard pairing of $m\in M$ and $n\in N$ as $n \cdot m$. We write $M_{\RR} = M\otimes\RR$, $N_{\RR} = N\otimes\RR$. Taking $\Bbbk$ to be a field of characteristic 0, for $m\in M$ we will write $x^m$ for the monomial in $\Bbbk[M]$.  For notational convenience we will write $x_1=x^{(1,0)}$ and $x_2=x^{(0,1)}$, thus we may identify $x^m$ with $x_1^{m_1}x_2^{m_2}$.

Recall from the previous section the lattice $M \cong \ZZ^2$ such that $\ZZ[M] \cong \ZZ[x_1^{\pm 1}, x_2^{\pm 1}]$.  We write $N = \Hom (M, \ZZ)$ for its dual lattice, $M_{\RR} = M\otimes\RR$, $N_{\RR} = N\otimes\RR$, and denote the standard pairing of $m\in M$ and $n\in N$ by $n \cdot m$.  Given a strictly convex rational cone $\sigma \subsetneq M_{\RR}$, we write $P=\sigma \cap M$.  For a fixed field $\Bbbk$ of characteristic zero, let $\widehat{\Bbbk[P]}$ denote the completion of the monoid ring $\Bbbk[P]$ at the maximal monomial ideal $\fm$ generated by $\left\{x^m \,|\, m\in P\smallsetminus\{0\}\right\}$. \sayHW{Unless it would be mathematically incorrect to do so, all $\Bbbk$'s in the paper should be $\ZZ$'s}
...
\begin{theorem} 
  \label{univLaurent} 
  Let $\fD:=\fD_{(b,c)}$ be as constructed above, and let $f \in \Bbbk[M]$ be a
  ...
  chambers of $\fD$, $\theta_{\gamma,\fD}(f)$ in fact lies in $\Bbbk[M]$.
\end{theorem}
...
\begin{defn} 
  ...
  $c(\ell) x^{m(\ell)} \in \Bbbk[M]$ for each domain of linearity $\ell \subseteq ( -
  \infty, 0]$ of $\gamma$. 
...
The proof will be to show that $\Theta_{q,m}^{\mathbf{d}}$ is a pointed element with the same Newton polygon as $x[-m_1,-m_2]$.  By Theorem \ref{th:Newton polygons}, this is already enough to show that $\Theta_{q,m}^\mathbf{d}=x[-m_1,-m_2]$.
...
\sayGM{This proof needs: a restatement of Theorem \ref{th:Newton polygons} which doesn't say `pointed' and only requires the right inequalities.}

If $m=(m_1,m_2)$ such that $m_1\leq0$, $m_2\leq0$, $m_1\geq bm_2$, or $m_2\geq cm_1$, then $\Theta_{q,m}^\mathbb{d}$ is the cluster monomial $x[-m_1,-m_2]$.

Next, assume that $m=(m_1,m_2)$ such that
\[ 0<m_1<bm_2,\;\;\; 0<m_2<am_1\]
...
\[ m_1\leq a_1\leq \left(\frac{m_1}{m_2}-b\right)a_2,\;\;\; m_2\leq a_2\leq \left(\frac{m_2}{m_1}-b\right)a_1\]
...
Since $\Theta_{q,m}^\mathbf{d}\in \mathcal{A}_{b,c}$, Theorem \ref{th:Newton polygons} implies that $\Theta_{q,m}^\mathbf{d}$ is a scalar multiple of $x[-m_1,-m_2]$. 
