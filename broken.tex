...
Given a wall $(\fd, f_{\fd})$ and a direction $v\in M$ transversal to $\fd$, we associate the element $\frakp_{v,\fd}\in
{\mathrm{Aut}}_{\ZZ-alg}\left(\widehat{\ZZ[P]}\right)$ defined by
\[
  \frakp_{v,\fd} (x^m) := x^m f_{\fd}^{m\cdot n }, 
\]
...
\]
transverse to each wall of $\fD$ that it crosses and with endpoints not in the support of $\fD$. We define the \textit{path-ordered product} $\frakp_{\gamma, \fD}$ along $\gamma$ as follows. For each power $k \geq 1$, let  
\[
...
\begin{theorem} 
  \label{univLaurent} 
  Let $\fD:=\fD_{(b,c)}$ be as constructed above and consider a Laurent polynomial $f \in \ZZ[M]$. For any path $\gamma$ for which $\frakp_{\gamma,\fD}$ is
  well-defined, $\frakp_{\gamma,\fD}(f)$ can be viewed as an element of
  ...
  chambers of $\fD$, $\frakp_{\gamma,\fD}(f)$ lies in $\ZZ[M]$, then $f$ is a universal
  Laurent polynomial.
\end{theorem}
...
\begin{theorem} $ $
  \begin{enumerate}
    \item
      ...
      path joining $q$ to $q'$, then $\frakp_{\gamma, \fD }(\vartheta_{q,m}) = \vartheta_{q', m}$. 
...
\begin{remark} 
  ...
  $\vartheta_{q,m}=\frakp_{\gamma,\fD_{(b,c)}}(x^{m})$ for a path $\gamma$
...
If $m=(m_1,m_2)$ such that $m_1\leq0$ or $m_2\leq0$, then $\vartheta_{q,m}^\mathbb{d}$ is the cluster monomial $x[-m_1,-m_2]$, by Remark \ref{rk:theta functions g-vector}.
