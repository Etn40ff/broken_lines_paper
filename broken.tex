...
\sayMC{Well defined just means we have the curve is nice, i.e.  not passing origin, transverse to the wall, 'smooth' when passing the wall. We can put it down but it is just a bit too much technical to me.}
\begin{defn}
...
If $m=(m_1,m_2)$ such that $m_1\geq0$ or $m_2\geq0$\sayGM{Should this be $m_1 \geq 0$ or $m_2 \geq 0$? (Gregg, Mark)}, then $\vartheta_{q,m}^\mathbb{d}$ is the cluster monomial $x[-m_1,-m_2]$, by Remark \ref{rk:theta functions g-vector}.
\sayMC{inequality changed. and get the 'next' one changed too! (or else it does not make sense to have both cases $\geq$}
